...
describe("Favorite", () => {
  ...
  describe("#create()", () => {
    ...
    it("should not create a favorite without an assigned post or user", (done) => {
      ...
      .then((favorite) => {
        // This block skips. Expectations written in catch.
        done();
      })
      .catch((err) => {
  ...
  describe("#setUser()", () => {
    ...
    it("should associate a favorite and a user together", (done) => {
      Favorite.create({ // Create favorite
        postId: this.post.id,
        userId: this.user.id
      })
      .then((favorite) => {
        this.favorite = favorite; // Store it
        expect(favorite.userId).toBe(this.user.id); // Confirm it's for this.user
        ...
        .then((newUser) => {
          this.favorite.setUser(newUser)  // Change favorite to newUser
          .then((favorite) => {
            expect(favorite.userId).toBe(newUser.id); // Confirm the update
            done();
          });
        })
        .catch((err) => {
  ...
  describe("#getUser()", () => {
    ...
    it("should return the associated user", (done) => {
      ...
      .then((favorite) => {
        favorite.getUser()
        .then((user) => {
          expect(user.id).toBe(this.user.id); // Confirm correct user is returned
          done();
        });
      })
      .catch((err) => {
