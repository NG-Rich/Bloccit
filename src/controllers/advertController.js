...
module.exports = {
  ...
  create(req, res, next) {
    ...
    advertQueries.addAdvert(newAdvert, (err, advert) => {
      if(err){
        ...
        res.redirect(303, `/advertise/${advert.id}`);
      }
    });
  },
  show(req, res, next) {
  ...
  destroy(req, res, next) {
    advertQueries.deleteAdvert(req.params.id, (err, advert) => {
      if(err) {
        res.redirect(500, `/advertise/${advert.id}`);
      }else {
        res.redirect(303, "/advertise")
      }
    });
  },
  edit(req, res, next) {
  ...
  update(req, res, next) {
    advertQueries.updateAdvert(req.params.id, req.body, (err, advert) => {
      if(err || advert == null) {
        res.redirect(404, `/advertise/${req.params.id}/edit`);
      }else {
        res.redirect(`/advertise/${advert.id}`);
      }
    });
  }
}
